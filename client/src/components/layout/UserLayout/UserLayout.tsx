    ...
    <div className="min-h-screen bg-background flex flex-col">
      ...
      <header className="sticky top-0 z-50 border-b bg-background/95 backdrop-blur supports-[backdrop-filter]:bg-background/60">
        <div className="container mx-auto flex h-16 items-center justify-between px-4">
          ...
          <nav className="hidden md:flex items-center gap-1">
            <NavLink to="/forms" isActive={isActive}>
              Forms
            </NavLink>
            <NavLink to="/my-responses" isActive={isActive}>
              My Responses
            </NavLink>
            <NavLink to="/profile" isActive={isActive}>
              Profile
            </NavLink>
          </nav>
            ...
            <nav className="flex flex-col gap-2">
              <Link
                ...
                className={`p-4 rounded-lg border hover:bg-accent transition-colors ${
                  isActive("/forms") ? "border-primary bg-primary/5" : ""
                }`}
              >
                Forms
              </Link>
              <Link
                ...
                onClick={() => setIsMobileMenuOpen(false)}
                className={`p-4 rounded-lg border hover:bg-accent transition-colors ${
                  isActive("/my-responses") ? "border-primary bg-primary/5" : ""
                }`}
              >
                My Responses
              </Link>
              <Link
                ...
                onClick={() => setIsMobileMenuOpen(false)}
                className={`p-4 rounded-lg border hover:bg-accent transition-colors ${
                  isActive("/profile") ? "border-primary bg-primary/5" : ""
                }`}
              >
                Profile
              </Link>
      ...
      <footer className="border-t bg-muted/30 mt-auto">
        <div className="container mx-auto px-4 py-6 text-center text-sm text-muted-foreground">
          <p>
            &copy; {new Date().getFullYear()} FormSuite. All rights reserved.
          </p>
        </div>
      </footer>
    </div>
